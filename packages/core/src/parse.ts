...
function asArray(value: Json, context: string): Json[] {
  if (!Array.isArray(value)) {
    throw new ParseError(`parseFormData: expected an array at ${context}, got ${typeof value}`);
  }
  return value;
}
...
function toEntryId(rawId: Json): string {
  return `entry.${String(rawId)}`;
}

function mapOptions(rawOptions: Json): ChoiceOption[] | undefined {
...
function parseSubQuestions(rawSubs: Json): SubQuestion[] {
 ...
 */
export function parseFormData(data: unknown, opts: { formId?: string } = {}): FormSchema {
  ...
  if (!Array.isArray(container)) {
    throw new ParseError("parseFormData: expected data[1] (main container) to be an array");
  }

  const description = optionalString(container[0]);
  const rawQuestions = container[1];
  if (!Array.isArray(rawQuestions)) {
    throw new ParseError("parseFormData: expected data[1][1] (question list) to be an array");
  }
  ...
  const wrappedTitle =
    typeof container8 === "string"
      ...
      : Array.isArray(container8)
        ...
        : undefined;
  const title =
    typeof wrappedTitle === "string" ? wrappedTitle : typeof data[3] === "string" ? data[3] : "";
  ...
  for (const rawEntry of rawQuestions) {
    ...
    if (typeof typeCode !== "number") {
      throw new ParseError(`parseFormData: question ${String(id)} has a non-numeric type code`);
    }
    ...
    if (kind === "page_break") {
      sawPageBreak = true;
      sections.push({ title: entryTitle, description: entryDescription, questionIds: [] });
      continue;
    }
    ...
    const firstSub = subs[0] as SubQuestion;
    ...
    switch (kind) {
      ...
      case "grid": {
        const rows: GridRow[] = subs.map((sub) => {
          const label = Array.isArray(sub.extra) ? optionalString(sub.extra[0]) : undefined;
      ...
      case "date": {
        const flags = Array.isArray(firstSub.rest[7]) ? (firstSub.rest[7] as Json[]) : undefined;
        question.date = {
      ...
      case "time": {
        const flags = Array.isArray(firstSub.rest[6]) ? (firstSub.rest[6] as Json[]) : undefined;
    ...
    const currentSection = sections[sections.length - 1] as Section;
...
export function parseFormHtml(html: string): FormSchema {
