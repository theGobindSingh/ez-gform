 ...
 */
export function normalizeFormId(input: string): string {
  ...
  if (/^https?:\/\//i.test(trimmed) || trimmed.startsWith("docs.google.com")) {
    ...
    if (dIndex === -1 || dIndex + 1 >= parts.length) {
      throw new Error(`normalizeFormId: could not find form id in URL "${input}"`);
    }
  ...
  if (!BARE_ID_RE.test(trimmed)) {
    throw new Error(`normalizeFormId: "${input}" is not a recognizable Google Form id or URL`);
  }
  return trimmed;
}

export function formUrls(formId: string): { viewform: string; formResponse: string } {
